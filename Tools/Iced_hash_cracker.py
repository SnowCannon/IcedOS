...
def banner(): 
	print """ Iced_Hash_Cracker """ 
	
parser = argparse.ArgumentParser(description="Iced_Hash_Cracker CLI. Ice_Hash_Cracker is a great tool for cracking all Hashes.")
parser.add_argument('-f', '--file', help = 'Add wordlist to create Rainbow table', nargs = 1)
parser.add_argument('-p', '--pre', help = 'Prepend salt value into words of wordlist', nargs = 1)
parser.add_argument('-a', '--ap', help = 'Append salt value into words of wordlist', nargs = 1)
parser.add_argument('-h', '--hash', help = 'Crack Hash', nargs = 1)
parser.add_argument('-v', '--verbose',  help = 'Verbose Mode', action='stor_true')
...
if os.path.isdir('rainbow_hash'):
	...
	od.chdir('rainbow_hash/')
	
sql = sqlite3.connect('hashing.db')
sql.execute('create table if not exists(name text, md5 text, sha1 text, sha224 text, blake2s text, '
